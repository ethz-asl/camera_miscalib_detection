...
def summary_tensor(tensor):
    with tf.name_scope('IndividualTensors'):
        tf.summary.histogram(tensor.name, tensor, collections=['summary'])
...
def init_model(input_shape):
    ...
    training = tf.placeholder_with_default(tf.constant(True, dtype=tf.bool), shape=(), name="training")
    ...
    dense1_drop = tf.keras.layers.Dropout(rate=0.3, name='dense1_drop')(dense1, training=training)
    ...
    y_pred = tf.keras.layers.Dense(units=1, activation=None,
                                   kernel_initializer='glorot_uniform',
                                   use_bias=False, name="output")(dense2)
    ...
    with tf.name_scope('Summary'):
        ...
        tf.summary.histogram('dense1', dense1, collections=['summary']) 
        tf.summary.histogram('dense1_drop', dense1_drop, collections=['summary']) 
        tf.summary.scalar('training_flag', tf.cast(training, tf.float32), collections=['summary'])  
#        tf.summary.histogram('bn_gamma', tf.get_default_graph().get_tensor_by_name('dense_block_1/compfun1/bn/gamma:0'), collections=['summary'])
        print(tf.get_default_graph().get_tensor_by_name('dense_block_1/compfun1/bn/gamma:0'))
        for var in tf.global_variables():
            if ("moving_mean" in var.name) or ("moving_variance" in var.name):
                 tf.summary.histogram(var.name, var, collections=['summary'])
...
#            tf.summary.histogram(op.name, tf.keras.layers.Flatten()(op.values()[0]))
