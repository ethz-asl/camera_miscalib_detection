...
def init_model(input_shape):
    ...
    stdsq_featuremap = tf.reduce_mean(tf.square(dense_block_3-mean_featuremap), axis=0, name='stdsq_feature_map')
    consistency_loss = tf.reduce_mean(stdsq_featuremap, name='consistency_loss_mse')
    consistency_loss_lambda = tf.placeholder(dtype=tf.float32, shape=(None), name="consistency_loss_lambda")


    optimizer = tf.train.AdamOptimizer(learning_rate=1e-4)
    train_op = optimizer.minimize(loss + consistency_loss_lambda*consistency_loss, name='train_op')
    ...
    with tf.name_scope('Images'):
        ...
        tf.summary.image('stdsq_featuremap', stdsq_featuremap, collections=['summary'])
