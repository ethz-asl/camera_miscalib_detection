...
class Dataset(object):
    def __init__(self, index_csv, selector='', remove_mean=False, remove_std=False,
        ...
        # apply the selector
        folders['tags'].apply(lambda tags: apply_selector(tags, selector))
        folders = folders[folders['tags']==True]
        ...
        data = pd.DataFrame(columns=['image_name', 'cal_group', 'width', 'height', 'fx', 'cx', 'fy', 'cy', 'k1',
                                     'k2', 'p1', 'p2', 'k3'])
        ...
        if self.verbose > 0:
            print("%d images found in %d folders group in %d groups from index file %s, when applying selector '%s'." %
    ...
    def get_outputs(self, ids, raw=False):
        ...
        for id in ids:
            ...
            appd, diff_map = miscal.appd(reference=self.samplers[cal_group].reference,
                                         width=self.cal_groups[cal_group]['width'],
                                         height=self.cal_groups[cal_group]['height'],
    ...
    def initialize_samplers(self):
        ...
        for cal_group in self.cal_groups:
            ...
            reference = cm.Calibration(K=[cg['fx'], cg['fy'], cg['cx'], cg['cy']],
                                      D=[cg['k1'], cg['k2'], cg['p1'], cg['p2'], cg['k3']],
                                      width=cg['width'], height=cg['height'])


            # Establish the perturbation ranges
            # [!!!] This needs to be fine tuned depending on the dataset used
            ranges = {'fx': (0.95 * cg['fx'], 1.20 * cg['fx']),
                      'fy': (0.95 * cg['fy'], 1.20 * cg['fy']),
                      'cx': (0.95 * cg['cx'], 1.05 * cg['cx']),
                      'cy': (0.95 * cg['cy'], 1.05 * cg['cy']),
                      'k1': (0.85 * cg['k1'], 1.15 * cg['k1']),
                      'k2': (0.85 * cg['k2'], 1.15 * cg['k2']),
                      'p1': (0.85 * cg['p1'], 1.15 * cg['p1']),
                      'p2': (0.85 * cg['p2'], 1.15 * cg['p2']),
                      'k3': (0.85 * cg['k3'], 1.15 * cg['k3'])}

            # Initialize the sampler
            sampler = cm.UniformAPPDSampler(ranges=ranges, cal_width=cg['width'], cal_height=cg['height'],
                                            reference=reference, temperature=5, appd_range_dicovery_samples=2000,
                                            appd_range_bins=20, init_jobs=self.n_jobs,
                                            width=int(width / self.resolution_reduction_factor),
                                            height=int(height / self.resolution_reduction_factor),
                                            min_cropped_size=(int(width / self.resolution_reduction_factor / 1.5),
                                                              int(height / self.resolution_reduction_factor / 1.5)))
