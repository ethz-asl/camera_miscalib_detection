        ...
        for i in range(n_composite_functions):
            with tf.variable_scope('compfun%d' % (i + 1)):
                ...
                if add_bottleneck:
                    ...
                    bn = tf.keras.layers.BatchNormalization(axis=-1, name='btlnk_bn')
                    x_new = bn(concatenated, training=training)
                    for u in bn.updates: tf.add_to_collection(tf.GraphKeys.UPDATE_OPS, u)
 
                    x_new = tf.keras.layers.ReLU(name='btlnk_relu')(x_new)
#                    x_new = tf.keras.layers.ReLU(name='btlnk_relu')(concatenated)
                ...
                bn = tf.keras.layers.BatchNormalization(axis=-1, name='bn')
                batch_norm = bn(x_new, training=training) 
                for u in bn.updates: tf.add_to_collection(tf.GraphKeys.UPDATE_OPS, u)

                activation = tf.keras.layers.ReLU(name='relu')(batch_norm)
#                activation = tf.keras.layers.ReLU(name='relu')(x_new)
        ...
        with tf.variable_scope('transition'):
            ...
            bn = tf.keras.layers.BatchNormalization(axis=-1, name='bn')
            output = bn(concatenated, training=training)
            for u in bn.updates: tf.add_to_collection(tf.GraphKeys.UPDATE_OPS, u)

            output_channels = int(np.floor(transition_compression_factor * output.get_shape().as_list()[1]))
#            output_channels = int(np.floor(transition_compression_factor * concatenated.get_shape().as_list()[1]))
...
#            output = tf.keras.layers.Conv2D(filters=output_channels, kernel_size=1,
#                                            padding='same', activation=None, use_bias=True,
#                                            kernel_initializer='glorot_uniform', name='conv')(concatenated)
